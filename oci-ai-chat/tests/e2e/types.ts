...
export interface BrowserTestContext {
  ...
  browser_click: (params: { selector: string; index?: number; tabId?: number; timeoutMs?: number }) => Promise<unknown>;
  browser_type: (params: { selector: string; text: string; clear?: boolean; tabId?: number }) => Promise<unknown>;
  ...
  browser_scroll: (params: { selector?: string; x?: number; y?: number; tabId?: number }) => Promise<unknown>;
}
