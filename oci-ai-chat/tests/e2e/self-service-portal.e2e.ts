...
async function waitForAIResponse(
  ...
  while (Date.now() - startTime < timeout) {
    ...
    if (!typing?.exists) {
      ...
      const stillTyping = await ctx.browser_query({ selector: SELECTORS.typingIndicator, mode: 'exists' });
    ...
    
    {
      ...
      async run(ctx) {
        ...
        if (!userName?.includes('Alex')) {
          return { passed: false, error: `Expected user name containing "Alex", got "${userName}"` };
        }
  ...
  tests: [
    {
      ...
      async run(ctx) {
        ...
        if (quickActionCount < 4) {
          return { passed: false, error: `Expected at least 4 quick actions, found ${quickActionCount}` };
        }
  ...
  tests: [
    ...
    
    {
      ...
      async run(ctx) {
        ...
        const asksQuestions = responseText && (
        ...
        if (!asksQuestions) {
          ...
          return { 
            passed: false, 
            error: 'AI did not ask clarifying questions before proceeding. This is the CRITICAL behavior we need to fix.',
  ...
  tests: [
    ...
    
    {
      ...
      async run(ctx) {
        ...
        const gathersRequirements = responseText && (
          responseText.toLowerCase().includes('vcpu') ||
          responseText.toLowerCase().includes('memory') ||
          responseText.toLowerCase().includes('region') ||
    ...
    
    {
      ...
      async run(ctx) {
        ...
        if (toolCardCount === 0) {
          ...
          const responseText = await getElementText(ctx, `${SELECTORS.assistantMessage}:last-child .assistant-text`);
  ...
  tests: [
    ...
    
    {
      ...
      async run(ctx) {
        ...
        if (messageCount < 4) { // 2 user + 2 assistant minimum
