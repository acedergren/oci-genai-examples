...
Full reference: `.claude/reference/framework-notes.md` (Fastify 5 decorators/hooks/streaming/testing, Vitest 4 config, RAG pipeline).

Critical guardrails (keep in context):

- **Plugin order is load-bearing**: error-handler → ... → session → RBAC. See `app.ts:68-129`.
- **Never `decorate('foo', null)`** — type is locked at creation. Pass real value.
- **Never `reply.send(undefined)`** — use `reply.code(204).send()`.
- **Vitest**: `defineProject` not `defineConfig` for workspace members.
