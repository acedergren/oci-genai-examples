...
import {
  WorkflowExecutor,
  type EngineState,
} from "../mastra/workflows/executor.js";
...
const workflowRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const workflow = await workflows.updateForUser(
        request.params.id,
        request.body,
        userId,
      );
    ...
    async (request, reply) => {
      ...
      if (definition.status !== "published" && definition.status !== "draft") {
        throw new ValidationError(
          "Only published or draft workflows can be executed",
          { workflowId: request.params.id, status: definition.status },
        );
      }
      ...
      try {
        ...
        await runs.updateStatus(run.id, {
          ...
          engineState: result.engineState as
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (run.definitionId !== request.params.id) {
        return reply
          .code(404)
          .send({ error: "Workflow run not found for this workflow" });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const definition = await workflows.getByIdForUser(
        run.definitionId,
        userId,
      );
