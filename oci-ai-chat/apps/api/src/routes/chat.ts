...
import { ValidationError } from "@acedergren/portal-shared";
import {
  ...
  fastify.withTypeProvider<ZodTypeProvider>().post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        request.log.error(
          { err },
          "Failed to load AI provider registry — using fallback",
        );
        useFallback = true;
      }

      const allowlist = useFallback ? FALLBACK_MODEL_ALLOWLIST : enabledModels;
      const effectiveDefault = allowlist.includes(DEFAULT_MODEL)
        ? DEFAULT_MODEL
        : allowlist[0];
      const model =
        requestedModel && allowlist.includes(requestedModel)
          ? requestedModel
          : effectiveDefault;
