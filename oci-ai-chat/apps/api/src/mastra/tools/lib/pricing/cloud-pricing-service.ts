...
export class OCIPricingClient {
  ...
  async calculateMonthlyCost(
    config: OCIComputeCostConfig,
  ): Promise<OCIComputeCostResult> {
    ...
    let ocpuCost =
      pricing.ocpuPricePerHour * config.ocpus * config.hoursPerMonth;
    let memoryCost =
      pricing.memoryGBPricePerHour * config.memoryGB * config.hoursPerMonth;
...
export class AzurePricingClient {
  ...
  async calculateMonthlyCost(
    ...
    if (!pricing) {
      throw new Error(
        `Unknown SKU: ${config.skuName} in region ${config.region}`,
      );
    }
...
export class AWSPricingClient {
  ...
  async listEC2Instances(filter?: {
    ...
    if (filter?.architecture) {
      instances = instances.filter(
        (i) => i.architecture === filter.architecture,
      );
    }
    return instances;
  }
  ...
  async calculateMonthlyCost(config: AWSCostConfig): Promise<AWSCostResult> {
    const instance = await this.getEC2Pricing(
      config.instanceType,
      config.region,
    );
    if (!instance) {
...
export class CloudPricingService {
  ...
  formatAsMarkdown(comparison: CloudComparison): string {
    ...
    if (requirements.networking?.egressGBPerMonth) {
      lines.push(
        `- **Egress**: ${requirements.networking.egressGBPerMonth} GB/month`,
      );
    }
    lines.push("");
    ...
    if (estimates.oci) {
      lines.push(
        `**Monthly Total: $${estimates.oci.monthlyTotal.toFixed(2)}**`,
      );
      lines.push("");
    ...
    if (estimates.azure) {
      lines.push(
        `**Monthly Total: $${estimates.azure.monthlyTotal.toFixed(2)}**`,
      );
      lines.push("");
      lines.push("| Service | Monthly Cost |");
      lines.push("|---------|-------------|");
    ...
    if (estimates.aws) {
      lines.push(
        `**Monthly Total: $${estimates.aws.monthlyTotal.toFixed(2)}**`,
      );
      lines.push("");
      lines.push("| Service | Monthly Cost |");
      lines.push("|---------|-------------|");
    ...
    if (recommendation.costSavingsPercent > 0) {
      lines.push(
        `- **Savings**: ${recommendation.costSavingsPercent.toFixed(1)}% lower cost`,
      );
    }
    lines.push("");
  ...
  private async estimateOCICost(
    ...
    if (requirements.compute) {
      ...
      if (pricing) {
        ...
        const withinFreeTier =
          isARM &&
          ocpus <= freeTier.compute.armOcpus &&
          memoryGB <= freeTier.compute.memoryGB;
    ...
    if (requirements.storage) {
      const storageType =
        requirements.storage.type === "ssd" ? "block-ssd" : "block-hdd";
      const storagePricing =
        await this.ociClient.getStoragePricing(storageType);

      if (storagePricing) {
        const storageCost =
          storagePricing.pricePerGBMonth * requirements.storage.sizeGB;
        breakdown.push({
          category: "storage",
    ...
    if (requirements.networking?.egressGBPerMonth) {
      ...
      const billableEgress = Math.max(
        0,
        requirements.networking.egressGBPerMonth - freeEgressGB,
      );
      ...
      if (billableEgress > 0) {
        ...
        notes.push(
          `Egress is within free tier (${freeTier.networking.egressTBFree}TB/month free)`,
        );
      }
    }
  ...
  private async estimateAzureCost(
    requirements: WorkloadRequirements,
  ): Promise<CostEstimate> {
    const breakdown: CostEstimate["breakdown"] = [];
    const notes: string[] = [];
  ...
  private async estimateAWSCost(
    ...
    if (requirements.networking?.egressGBPerMonth) {
      ...
      if (billableEgress > 0) {
        ...
        notes.push(
          `Egress is within AWS free tier (${AWS_FREE_EGRESS_GB}GB/month free)`,
        );
      }
    }
  ...
  private generateRecommendation(
    ...
    if (requirements.constraints?.preferredProvider) {
      ...
      const preferredCost =
        preferred === "oci"
          ? ociEstimate.monthlyTotal
          : azureEstimate.monthlyTotal;
      const otherCost =
        preferred === "oci"
          ? azureEstimate.monthlyTotal
          : ociEstimate.monthlyTotal;
    ...
    if (requirements.constraints?.maxBudgetPerMonth) {
      ...
      if (
        ociEstimate.monthlyTotal > budget &&
        azureEstimate.monthlyTotal > budget
      ) {
        reasoning.push(`Both providers exceed budget of $${budget}/month`);
        reasoning.push(
          "Consider reducing requirements or using reserved instances",
        );
      } else if (ociEstimate.monthlyTotal > budget) {
        reasoning.push(
          `OCI exceeds budget, Azure fits within $${budget}/month`,
        );
      } else if (azureEstimate.monthlyTotal > budget) {
        reasoning.push(
          `Azure exceeds budget, OCI fits within $${budget}/month`,
        );
      }
    }
    ...
    if (requirements.compute?.architecture === "arm") {
      ...
      if (recommendedProvider === "oci") {
        reasoning.push(
          "OCI A1.Flex includes 4 OCPUs and 24GB RAM in Always Free tier",
        );
      }
    }
    ...
    if (requirements.networking?.egressGBPerMonth) {
      ...
      if (egress > 100) {
        reasoning.push(
          `OCI includes 10TB/month free egress vs Azure's 5GB free`,
        );
        if (egress <= 10000) {
    ...
    if (recommendedProvider === "oci") {
      ...
      tradeoffs.push(
        "OCI has more generous always-free tier and 10TB/month free egress",
      );
    } else {
      tradeoffs.push(
        "OCI offers better Oracle database integration and 10TB free egress",
      );
