 ...
 */
export const DESTRUCTIVE_TOOL_WARNINGS: Record<
  string,
  { warning: string; impact: string }
> = {
  terminateInstance: {
    warning:
      "This will permanently delete the compute instance and all its data.",
    impact:
      "Instance will be unrecoverable. Boot volume may be preserved if specified.",
  },
  stopInstance: {
  ...
  deleteVcn: {
    warning:
      "This will delete the Virtual Cloud Network and all associated resources.",
    impact:
      "All subnets, route tables, and security lists in this VCN will be deleted.",
  },
  deleteBucket: {
    warning: "This will permanently delete the Object Storage bucket.",
    impact:
      "All objects in the bucket will be deleted. This action is irreversible.",
  },
  terminateAutonomousDatabase: {
    warning: "This will permanently terminate the Autonomous Database.",
    impact:
      "All data in the database will be lost. Backups may still be available.",
  },
};
 ...
 */
export function getToolWarning(
  toolName: string,
): { warning: string; impact: string } | undefined {
