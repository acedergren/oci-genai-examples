...
export const observabilityTools: ToolEntry[] = [
  {
    ...
    execute: (args) => {
      ...
      if (args.displayName)
        cliArgs.push("--display-name", args.displayName as string);
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      namespace: z
        .string()
        .describe(
          "Metric namespace (e.g., oci_computeagent, oci_vcn, oci_blockstore)",
        ),
      query: z
        .string()
        .describe('MQL query string, e.g. "CpuUtilization[1h].mean()"'),
      startTime: z
        .string()
        .optional()
        .describe("Start time (ISO 8601) — defaults to 3 hours ago"),
      endTime: z
        .string()
        .optional()
      ...
      resolution: z
        .string()
        .optional()
        .describe("Data resolution (e.g., 1m, 5m, 1h)"),
    }),
    execute: (args) => {
      const compartmentId = requireCompartmentId(args);
      const now = new Date();
      ...
      const startTime =
        (args.startTime as string) || threeHoursAgo.toISOString();
      ...
      if (args.resolution)
        cliArgs.push("--resolution", args.resolution as string);
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      instanceId: z
        .string()
        .optional()
        .describe("Filter to a specific instance OCID"),
      aggregation: z
    ...
    executeAsync: async (args) => {
      ...
      const startTime = new Date(
        now.getTime() - config.hoursBack * 60 * 60 * 1000,
      );
