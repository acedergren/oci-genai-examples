...
export const storageTools: ToolEntry[] = [
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      publicAccessType: z
        .enum(["NoPublicAccess", "ObjectRead"])
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      imageVersion: z
        .string()
        .optional()
        .describe("Filter by image version/tag"),
    }),
    execute: (args) => {
      const compartmentId = requireCompartmentId(args);
      const cliArgs = [
      ...
      if (args.repositoryName)
        cliArgs.push("--repository-name", args.repositoryName as string);
      if (args.imageVersion)
        cliArgs.push("--display-name", args.imageVersion as string);
