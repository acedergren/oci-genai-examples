...
import { OCIError } from "@acedergren/portal-shared";
...
export function requireCompartmentId(args: Record<string, unknown>): string {
  const compartmentId =
    (args.compartmentId as string) || getDefaultCompartmentId();
  if (!compartmentId) {
    throw new OCIError(
      "No compartmentId provided and OCI_COMPARTMENT_ID not set",
      {
        field: "compartmentId",
      },
    );
...
export function slimOCIResponse(data: unknown, pickFields: string[]): unknown {
  ...
  if (Array.isArray(obj.data)) {
    return {
      data: obj.data.map((item: Record<string, unknown>) =>
        pick(item, pickFields),
      ),
    };
  }
