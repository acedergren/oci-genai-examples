...
function createMockWithConnection(mockConn = createMockConnection()) {
  const withConnection = vi.fn(async (fn: (conn: typeof mockConn) => unknown) =>
    fn(mockConn),
  );
...
describe("OracleVectorStore", () => {
  ...
  describe("createIndex", () => {
    it("creates a new table with VECTOR column when table does not exist", async () => {
      ...
      expect(mockConn.execute).toHaveBeenCalledWith(
        expect.stringContaining("user_tables"),
      ...
      expect(mockConn.execute).toHaveBeenCalledWith(
        expect.stringContaining(
          "CREATE VECTOR INDEX idx_mastra_vector_test_embeddings_vec",
        ),
      );
  ...
  describe("upsert", () => {
    ...
    it("handles deleteFilter before upserting", async () => {
      ...
      expect(mockConn.execute).toHaveBeenCalledWith(
        expect.stringContaining(
          "DELETE FROM MASTRA_VECTOR_TEST_EMBEDDINGS WHERE",
        ),
    ...
    it("returns prefixed tables plus legacy table if exists", async () => {
      mockConn.execute
        .mockResolvedValueOnce({
          rows: [
            { TABLE_NAME: "MASTRA_VECTOR_DOCS" },
            { TABLE_NAME: "MASTRA_VECTOR_CHAT" },
          ],
        })
  ...
  describe("deleteIndex", () => {
    ...
    it("refuses to drop legacy conversation_embeddings table", async () => {
      await expect(
        store.deleteIndex({ indexName: "conversation_embeddings" }),
      ).rejects.toThrow(/Cannot delete legacy index/);
    });
  });
  ...
  describe("deleteVector", () => {
    it("deletes a single vector by ID", async () => {
      ...
      expect(mockConn.execute).toHaveBeenCalledWith(
        expect.stringContaining(
          "DELETE FROM MASTRA_VECTOR_TEST_EMBEDDINGS WHERE id = :id",
        ),
        { id: "v1" },
      );
    });
