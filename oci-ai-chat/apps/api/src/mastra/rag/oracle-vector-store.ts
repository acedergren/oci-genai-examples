...
function validateTableName(name: string): void {
  if (!SAFE_IDENTIFIER.test(name)) {
    throw new Error(
      `Invalid vector index name: "${name}". Only A-Z, 0-9, and _ are allowed.`,
    );
  }
  if (name.length > 128) {
    throw new Error(
      `Vector index name too long: ${name.length} chars (max 128).`,
    );
  }
}
...
export class OracleVectorStore extends MastraVector<Record<string, unknown>> {
  ...
  async upsert(params: UpsertVectorParams): Promise<string[]> {
    ...
    const generatedIds: string[] = ids?.length
      ? [...ids]
      : vectors.map(() => crypto.randomUUID());

    await this.withConnection(async (conn) => {
      // Handle deleteFilter atomically before insert (only flat key-value filters supported)
      const df = params.deleteFilter as
