...
describe("safeEvaluateExpression", () => {
  it("evaluates equality", () => {
    expect(safeEvaluateExpression('status == "ok"', { status: "ok" })).toBe(
      true,
    );
    expect(safeEvaluateExpression('status == "fail"', { status: "ok" })).toBe(
      false,
    );
  });
  ...
  it("evaluates dot-path on left side", () => {
    ...
    expect(safeEvaluateExpression("result.data.length > 0", context)).toBe(
      true,
    );
  });
  ...
  it("resolves path references on right side", () => {
    expect(safeEvaluateExpression("a == b", { a: "hello", b: "hello" })).toBe(
      true,
    );
  });
});
